...
def main():
    ...
    corpus_path= "/home/ryougiguda/Projects/multilingual_nmt/data/bleu_single_pair/data_for_bpe_learning/" #path of corpus 
    code_switched_path= f"/home/ryougiguda/Projects/multilingual_nmt/data/bleu_single_pair/data_code_switching/code_switching{num}/"#path of code_switched data
    if os.path.exists(dict_file)!=True:  #如果没有对应词典，则直接将源文件复制过去
        source_file = corpus_path+lang_pairs+"."+src
        destination_file =code_switched_path+lang_pairs+"."+src
    ...
    if os.path.exists(corpus_path+lang_pairs+"."+src):
        print(corpus_path+lang_pairs+"."+src)
        with open(corpus_path+lang_pairs+"."+src, "r") as src_corpus, \
             open(corpus_path+lang_pairs+"."+tgt, "r") as tgt_corpus, \
             open(code_switched_path+lang_pairs+"."+src, "w") as src_switched:
            for src_line in src_corpus:
                ...
                src_switched.write("\n")
    print("finished switching "+corpus_path+"."+lang_pairs+"."+src)
