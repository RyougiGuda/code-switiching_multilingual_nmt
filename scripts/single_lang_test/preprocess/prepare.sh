...
symlink_path=$data_root/single_lang_test/symlink_path #经过symlink处理之后的双向数据
moses_path=$data_root/single_lang_test/moses_path #经过mosesdecoder处理后
data_for_bpe_learning=$data_root/single_lang_test/data_for_bpe_learning #经过初步处理后，用于bpe学习
bpe_path=$data_root/single_lang_test/bpe_path #经过bpe处理后
add_tokens_path=$data_root/single_lang_test/add_tokens_path #经过add_tokens处理后
merged_path=$data_root/single_lang_test/merged_path #经过merged处理后
processed_path=$data_root/single_lang_test/processed_path #经过preprocess函数处理后
BPE_out=$data_root/single_lang_test/bpe #存储学习到的bpe model
BPESIZE=20000
